...
//
//function get_form_directory_uri(){  return get_template_directory_uri() . "/include/nik-form"; }
...
function assets_path($file){
  ...
  return get_template_directory_uri() . '/src/' . $file;
...
function nik_get_excerpt_by_post_id($post_id){
...
function nik_excerpt_length($length) {
    return 20;
}
add_filter('excerpt_length', 'nik_excerpt_length', 999);
...
function nik_lower_wpseo_priority( $html ) {
    return 'low';
}
add_filter( 'wpseo_metabox_prio', 'nik_lower_wpseo_priority' );
...
//Dropdown in admin
/*add_action('restrict_manage_posts', 'nik_filter_post_type_by_taxonomy');
function nik_filter_post_type_by_taxonomy() {
	global $typenow;
...
add_filter('parse_query', 'nik_convert_id_to_term_in_query');
function nik_convert_id_to_term_in_query($query) {
	global $pagenow;
	$post_type = ''; // insert to your post type
	$taxonomy  = ''; // insert to your taxonomy
